...
// };
  ...
  {/* Header */}
  <header className="text-center py-6 bg-yellow-100 shadow-md">
    <h1 className="text-5xl font-extrabold tracking-wider text-yellow-600">
      🏆 Claim Points Portal
    </h1>
    <p className="mt-2 text-yellow-800">Level up by claiming your daily rewards</p>
  </header>
        ...
        .map((user) => (
          <React.Fragment key={user._id}>
            ...
            <div>
              ...
              <p className="text-yellow-700 mt-1">Points: {user.userPoints}</p>
            </div>
          </React.Fragment>
        ))}
    </div>
...
>
  ...
  >
    ...
    <div className="flex flex-col sm:flex-row gap-4 justify-center">
      <NavLink
        to="/Leaderboard"
        className="bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-2 px-6 rounded-lg text-center"
      >
        View Leaderboard
      </NavLink>
      <NavLink
        to="/History"
        className="bg-yellow-700 hover:bg-yellow-800 text-white font-bold py-2 px-6 rounded-lg text-center"
      >
        View Claim History
      </NavLink>
    </div>
